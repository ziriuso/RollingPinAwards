...
- Receiving `sync_hello` answers with a targeted `WHISPER` full flat record stream for rank permissions, aliases, awards, nominations, votes, and hidden delete tombstones, followed by `sync_snapshot_complete`. Snapshot replies are sent only when the requester is currently online in the guild roster, abort on the first transport send failure, use `BULK` priority, and are debounced per requester for 30 seconds. Awards are sent before nominations so clients can reject old pending nomination replays that were already approved.
