...
- `Bootstrap.lua` requests the guild roster on enable, sends a lightweight `sync_hello` once per active guild when sync enables, and `Core/Events.lua` sends again after guild context appears later through `PLAYER_GUILD_UPDATE`.
- If the client first sees only a provisional name-based guild key and later resolves the stable guild club id, `Bootstrap.lua` migrates the guild dataset key and sends a fresh peer-discovery `sync_hello` for the stable key.
