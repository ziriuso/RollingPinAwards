...
- `RollingPinAwards/RollingPinAwards.toc`: keep `## Interface:` aligned with supported WoW clients. For the 1.1.0 release this is `120007, 120005`.
...
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\release\Build-CurseForgePackage.ps1 -TagName v1.1.0 -OutputDirectory .\artifacts\release
```
...
git commit -m "chore: prepare 1.1.0 curseforge release"
...
```powershell
gh release view v1.1.0 --json name,tagName,isPrerelease,assets,url
```

The stable release should have `isPrerelease: false`, a `RollingPinAwards-1.1.0.zip` asset, and a successful CurseForge upload step in the workflow log.
