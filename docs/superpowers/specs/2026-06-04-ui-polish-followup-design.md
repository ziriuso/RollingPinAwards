...
- Position the full six-button navbar with the 188px sizing margin, then align the Nominations/History gap with the Dashboard `Top Recipient`/`Pending Nominations` gap.
- Position the five-button navbar, when Admin is hidden, so the Nominations button lands on that same Dashboard center gap.
...
- Navbar layout uses the main background art dimensions as the measurement frame. It derives the rendered button width from the visible tab count, configured gaps, and the requested background-edge margins; both Admin-visible and no-Admin layouts use the Dashboard center gap as the middle nav focus.
