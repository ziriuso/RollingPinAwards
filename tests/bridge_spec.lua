...
return {
  ...
  ["dashboard renders stats, content sections, and footer actions after recomposition"] = function()
    ...
    harness.assert_equal("Rolling Pins", panel.statCards[1].label.text)
  ...
  ["dashboard recent awards clip long reasons and open a detail popup"] = function()
    ...
    harness.assert_equal("Zirleficent", row.label.text)
    harness.assert_true(row.secondaryLabel ~= nil)
    harness.assert_true(row.secondaryLabel.text:match(longReason) == nil)
    harness.assert_true(row.secondaryLabel.text:match("%.%.%.") ~= nil)
    harness.assert_true(row.secondaryLabel.point and row.secondaryLabel.point[2] == row.label)
    harness.assert_equal("BOTTOMLEFT", row.secondaryLabel.point and row.secondaryLabel.point[3])
    harness.assert_equal("TOPLEFT", row.label.point and row.label.point[1])
    harness.assert_false(row.label.wordWrap)
    harness.assert_true(row.label.text:match("Awarded by") == nil)
